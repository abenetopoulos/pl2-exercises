...
main = do
    ...
    let sep = L.lines s
        --seps = get_tuple sep
        graph = generate_graph (tail sep) Map.empty
...
get_tuple :: L.ByteString -> (Int, Int)
get_tuple s =
    case (L.readInt s) of
    Nothing -> (-1, -1)
    Just (fNode, rest) ->
...
generate_graph :: [L.ByteString] -> Graph -> Graph
generate_graph [] acc = acc
generate_graph (s:ss) acc =
    let (l, r) = get_tuple s
        get_val k =
        ...
        lList = get_val l
        rList = get_val r
        newAcc = Map.insert l (r : lList) $ Map.insert r (l : rList) acc
    in newAcc `seq` generate_graph ss newAcc
