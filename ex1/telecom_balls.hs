...
search_on_peds [] _ time _ times candidateLowPoint maxLowPoint = (time, times, candidateLowPoint, maxLowPoint)
search_on_peds (n : ns) parent time graph times candidateLowPoint maxLowPoint =
    case (Map.lookup n times) of
    Nothing ->
        ...
        in
            ...
            Just neighbours ->
                let (finalTime, auxTimes, childrenLowPoint, maxChildrenLowPoint) = search_on_peds neighbours n (time + 1) graph tempTimes (time + 1) 0
                ...
                in latestTimes `seq` search_on_peds ns parent (finalTime + 1) graph latestTimes (min candidateLowPoint nLowPoint) (max maxLowPoint nLowPoint)
    Just ((start, _), _, _) ->
        case (Map.lookup parent times) of
        ...
        Just (_, par, _) ->
            if (par == n) then
                ...
                search_on_peds ns parent time graph times candidateLowPoint maxLowPoint
            else
                --parent has a back edge, we should signify this
                search_on_peds ns parent time graph times (min start candidateLowPoint) maxLowPoint
