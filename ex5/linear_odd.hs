...
instance Show E where
  showsPrec p (Literal c) =
    ('#' :) . (c :)
  showsPrec p (Write e1 e2) =
    ('+' :) . showsPrec p e1 . showsPrec p e2
  showsPrec p (Read e) =
    ('-' :) . showsPrec p e
  showsPrec p (Variable x) =
    (x :)
  showsPrec p (Lambda x e) =
    ('/' :) . (x :) . showsPrec p e
  showsPrec p (Application e1 e2) =
    ('@' :) . showsPrec p e1 . showsPrec p e2
-}
...
sem phrase env =
    case phrase of
    Literal c -> --error "not implemented : sem - Literal"
        let
            functor s =
                (Character c, s)
        in
            functor
    Write e1 e2 -> --error "not implemented : sem - Write"
        let
        ...
        in
            functor'
    Read e -> --error "not implemented : sem - Read"
        let
            eFunctor = sem e env
        ...
        in
            functor
    Variable x -> --error "not implemented : sem - Variable"
        let
            functor s =
        ...
        in
            functor
    Lambda arg body -> --error "not implemented : sem - Lambda"
        let
            argFunctor v =
                let
        ...
        in
            functor
    Application e1 e2 -> --error "not implemented : sem - Application"
        let
            e1Functor = sem e1 env
            e2Functor = sem e2 env
            functor s =
                let
                    (e2Val, e2State) = e2Functor s
                in
                    case e2Val of
                    Character x ->
                        let
                            res @ (e1Val, e1State) = e1Functor e2State
                        in
                            case e1Val of
                            Complex f -> f e2Val e2State
                            Character x -> res
                    Complex g ->
                        let
                            aux v =
